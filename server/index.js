...
if (passport.googleEnabled) {
  ...
  app.get('/api/auth/google/callback',
    ...
    (req, res) => {
      try {
        // Get the frontend URL - THIS MUST WORK
        const frontendUrl = getPrimaryFrontendUrl();
        
        // Debug logging - check what we got
        console.log('🔍 OAuth Callback Debug:');
        console.log('  - Frontend URL (raw):', frontendUrl);
        console.log('  - CLIENT_URL env:', process.env.CLIENT_URL || 'NOT SET');
        console.log('  - User exists:', !!req.user);
        console.log('  - Token exists:', !!req.user?._jwtToken);
        
        // VALIDATE: Make sure frontendUrl doesn't contain "CLIENT_URL="
        if (frontendUrl.includes('CLIENT_URL=')) {
          console.error('❌ CRITICAL: frontendUrl contains "CLIENT_URL="');
          // Force the correct URL
          return res.redirect('https://shirtcraft-dob.vercel.app/login?error=bad_config');
        }
        ...
        // Force the correct URL as fallback
        res.redirect('https://shirtcraft-dob.vercel.app/login?error=callback_failed');
      }
    }
  );
} else {
...
const startServer = (port) => {
  ...
  server.listen(port, () => {
    ...
    // WARNING if frontendUrl is wrong
    if (frontendUrl.includes('CLIENT_URL=')) {
      console.error('❌ WARNING: CLIENT_URL is misconfigured! It contains "CLIENT_URL="');
      console.error('   Please set CLIENT_URL to just the URL, e.g., https://shirtcraft-dob.vercel.app');
    }
  });
};
